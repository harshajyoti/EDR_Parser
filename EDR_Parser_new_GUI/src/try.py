...
# --------------------------------------------- Assigns the available port --------------------------------------------- #

# def assign_port():
#     # print("inside assign port: ")
#     print("Data fetch Successful! Have a look in the browser using the below link")
#     print("------------------------------------------------------------------")
...
#     else:
#         print("No available ports within the specified range.")

# ------------------------------------------------------ web page ------------------------------------------------------ #

data_file = pkg_resources.resource_filename(__name__, 'templates/data.html')
diss_supi_file = pkg_resources.resource_filename(__name__, 'templates/diss_supi.html')
home_file = pkg_resources.resource_filename(__name__, 'templates/home.html')
index_file = pkg_resources.resource_filename(__name__, 'templates/index.html')
rat_type_file = pkg_resources.resource_filename(__name__, 'templates/rat_type.html')
rat_type_supi_file = pkg_resources.resource_filename(__name__, 'templates/rat_type_supi.html')
reasons_file = pkg_resources.resource_filename(__name__, 'templates/reasons.html')
snssai_file = pkg_resources.resource_filename(__name__, 'templates/snssai.html')
snssai_supi_file = pkg_resources.resource_filename(__name__, 'templates/snssai_supi.html')
dnn_file = pkg_resources.resource_filename(__name__, 'templates/dnn.html')
dnn_supi_file = pkg_resources.resource_filename(__name__, 'templates/dnn_supi.html')

edr_app = Flask(__name__)

if len(sorted_supi_dict) == 0:
    data = info_dict
else:
    data = sorted_supi_dict

reasons = disconnect_reason_dict
rat_data = rat_type_dict
snssai_data = snssai_dict
dnn_data = dnn_dict
...
def home():
    length = len(data)
    keys = list(data.keys())
    return render_template("home.html", keys=keys, length = length)

@edr_app.route("/data/<supi>")
def data_page(supi):
    if supi in data:
        values = data[supi]
        return render_template("data.html", supi=supi, values=values)
    else:
        return "Error: Supi not found."

# ---------------------------------------------- Disconnect Reasons -------------------------------------------------- #

@edr_app.route("/Disconnect_reasons")
def diss_reason():
    length = len(reasons)
    keys = list(reasons.keys())
    return render_template("reasons.html", keys=keys, length = length)

@edr_app.route("/reasons/<keys>")
def diss_reason_supi(keys):
    if keys in reasons:
        keys = reasons[keys]
        return render_template("diss_supi.html", keys=keys)
    else:
        return "Error: Supi not found."

# ---------------------------------------------------- Rat-type ------------------------------------------------------ #

@edr_app.route("/Rat-type")
def rat_type():
    length = len(rat_data)
    keys = list(rat_data.keys())
    return render_template("rat_type.html", keys=keys, length = length)

@edr_app.route("/<keys>")
def rat_type_supi(keys):
    if keys in rat_data:
        keys = rat_data[keys]
        return render_template("rat_type_supi.html", keys=keys)
    else:
        return "Error: Supi not found."

# ---------------------------------------------------- S-NSSAI ------------------------------------------------------ #

@edr_app.route("/snssai")
def snssai():
    length = len(snssai_data)
    keys = list(snssai_data.keys())
    return render_template("snssai.html", keys=keys, length = length)

@edr_app.route("/snssai-type/<path:keys>")
def snssai_supi(keys):
    if keys in snssai_data:
        supi_data = snssai_data[keys]
        return render_template("snssai_supi.html", keys=supi_data)
    else:
        return "Error: Supi not found."
...
def dnn():
    length = len(dnn_data)
    keys = list(dnn_data.keys())
    return render_template("dnn.html", keys=keys, length = length)

@edr_app.route("/dnn-type/<path:keys>")
def dnn_supi(keys):
    if keys in dnn_data:
        supi_data = dnn_data[keys]
        return render_template("dnn_supi.html", keys=supi_data)
    else:
        return "Error: Supi not found."

# --------------------------------- Assign the port according to the availability ------------------------------------- #
...
if __name__ == "__main__":
    ...
    filter_value = ""
    file_path = "C:\\Users\\hjyoti\\Desktop\\edr\\backup\\test_files\\smf-service-n0-0_0_20230604225157.transaction (1).csv"
